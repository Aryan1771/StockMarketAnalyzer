      ...
      <div className="card space-y-4">
        ...
        <form className="space-y-4" onSubmit={submit}>
          ...
          <label className="block">
            ...
            <input className="input mt-1" value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} placeholder="aryan123" />
          </label>
          <label className="block">
            <span className="muted">Password</span>
            <input className="input mt-1" type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} placeholder="At least 6 characters" />
          </label>
          <button className="btn w-full sm:w-auto">{mode === "login" ? "Login" : "Create account"}</button>
        </form>
      </div>
    </div>
  );
